...
class RestaurantRepository:
    ...
    def delete_restaurant(self, restaurant: Restaurant) -> None:
    ...
    def search_restaurants(
        ...
        if min_score is not None:
            ...
            query = query.filter(Restaurant.safety_score >= min_score)

        if max_score is not None:

            query = query.filter(Restaurant.safety_score <= max_score)

        if high_risk:

            query = query.filter(Restaurant.safety_score < 40)

        # Sorting

        if sort_by == "score":

            query = query.order_by(desc(Restaurant.safety_score))

        elif sort_by == "name":

            query = query.order_by(asc(Restaurant.name))
