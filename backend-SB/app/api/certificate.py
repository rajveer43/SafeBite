...
def map_certificate_type(val: str) -> CertificateType:
    ...
    if "fssai" in val_lower:
        ...
        for enum_item in CertificateType:
            if enum_item.value == val_lower or enum_item.name.lower() == val_lower:
                return enum_item
        return CertificateType.OTHER
...
async def create_certificate(
    ...
    try:
        if "multipart/form-data" in content_type:
            ...
            cert_num = str(form.get("certificate_number") or f"CERT-{uuid.uuid4().hex[:8].upper()}")
            issuing_auth = str(form.get("issuing_authority") or "Food Safety Authority")
            ...
            if restaurant_id_str and str(restaurant_id_str).strip():
                ...
                owner_restaurants = rest_repo.get_restaurants_by_owner(current_user.user_id)
...
def get_all_certificates(
    ...
    service = CertificateService(db)
    owner_id = current_user.user_id if current_user.role == "owner" else None
